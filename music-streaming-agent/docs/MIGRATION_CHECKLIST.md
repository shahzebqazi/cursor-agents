...
Use this when splitting or renaming remotes after moving generic docs into **my-cursor-config** (private).
...
- [ ] **`my-cursor-config`**: add or refresh `origin` → `shahzebqazi/my-cursor-config`. Pull `main` to pick up `patterns/` used by this repo’s docs. Local clone dir may still be `cursor-config/`.
- [ ] **Music server repo** (**my-pi-server-config**): ensure it documents how it pins this package (submodule SHA, subtree, or copy path) per [INTEGRATION.md](INTEGRATION.md).
- [ ] If you removed operator-specific scripts from the public tree, restore them in **my-cursor-config** or **my-pi-server-config** from git history before the cleanup commit.

## Docs and bookmarks

- [ ] Replace any old links to `docs/agents/GITHUB_PROJECTS_AND_SWE.md` **full prose** bookmarks: the file still exists but defers generic sections to **my-cursor-config** `patterns/`.
