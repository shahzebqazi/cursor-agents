...
**Editor-agnostic workflow habits** (Kanban, parallel sessions) and optional **Cursor rules** live in **[shahzebqazi/cursor-config](https://github.com/shahzebqazi/cursor-config)** under `patterns/` and `.cursor/rules/`. Link or vendor those files if you want the same guidance in this clone; they are not dependencies of the Python package.
